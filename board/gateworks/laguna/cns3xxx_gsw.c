...
int cns3xxx_init_mem(GswDev *dev)
{
	...
	for (i = 0; i < TX_SIZE; i++) {
		...
		dev->tx_ring[i].uco = 0;
		dev->tx_ring[i].tco = 0;
		flush_dcache_range((u32)&(dev->tx_ring[i]), (u32)&(dev->tx_ring[i]) + sizeof(TXDesc));
	}
	...
	for (i = 0; i < RX_SIZE; i++) {
		...
		flush_dcache_range((u32)&(dev->rx_ring[i]), (u32)&(dev->rx_ring[i]) + sizeof(TXDesc));
	}
...
static void cns3xxx_gsw_tx(GswDev *dev)
{
	...
	while ((pkt = cns3xxx_gsw_tx_pkt_dequeue(dev))) {
		...
		invalidate_dcache_range((uint32_t )tx_desc, (uint32_t)tx_desc + sizeof(TXDesc));
		if (!tx_desc->cown) {
		...
		flush_dcache_range((uint32_t)tx_desc, (uint32_t)tx_desc + sizeof(TXDesc));
		// point to next index
...
static int cns3xxx_eth_send(struct eth_device *edev, volatile void *packet, s32 length)
{
	...
	flush_dcache_range((uint32_t)pkt->pkt_buffer, (uint32_t)pkt->pkt_buffer + pkt->length);
...
static void cns3xxx_gsw_rx(GswDev *dev)
{
	...
	while (1) {
		...
		invalidate_dcache_range((uint32_t)rx_desc, (uint32_t)rx_desc + sizeof(rx_desc));
		if (rx_desc->cown == 0) {
			break;
		}
		...
		rx_desc->cown = 0;
		flush_dcache_range((u32)rx_desc, (u32)rx_desc + sizeof(rx_desc));

		invalidate_dcache_range((uint32_t)rcvpkt->pkt_buffer, (uint32_t)rcvpkt->pkt_buffer + rcvpkt->length);
